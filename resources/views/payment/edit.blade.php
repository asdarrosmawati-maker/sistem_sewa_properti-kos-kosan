<x-app>
    ...
    <div class="card shadow-lg p-3">
        ...
        <form action="{{ route('payment.update', $payment) }}" method="post" class="form" enctype="multipart/form-data">
            @csrf
            @method('put')
                ...
                <div class="col-md-12">
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('amount') is-invalid @enderror" type="number" id="amount"
                            name="amount" required min="0" value="{{ old('amount', $payment->amount) }}">
                        @error('amount')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('payment_date') is-invalid @enderror" type="date" id="payment_date"
                            name="payment_date" required value="{{ old('payment_date', $payment->payment_date) }}">
                        @error('payment_date')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                    ...
                    <div class="mb-3">
                        <label for="upload" class="form-label">Bukti Pembayaran</label>
                        <input class="form-control @error('proof_of_payment') is-invalid @enderror" type="file" id="upload"
                            name="proof_of_payment" accept="image/*">
                        <small class="text-muted d-block mt-1">Biarkan kosong jika tidak ingin mengubah bukti bayar.</small>
                        <img src="{{ $payment->proof_of_payment ? asset('storage/' . $payment->proof_of_payment) : asset('niceadmin/img/noprofil.png') }}" class="mt-2" id="preview" width="200" alt="">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select select2-default @error('status') is-invalid @enderror" id="status" name="status" required>
                            ...
                            <option value="Verified" @selected(old('status', $payment->status) == 'Verified')>Verified</option>
                            <option value="Rejected" @selected(old('status', $payment->status) == 'Rejected')>Rejected</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">
                                {{ $message }}
