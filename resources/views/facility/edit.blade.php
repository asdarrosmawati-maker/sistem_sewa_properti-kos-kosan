<x-app>
    ...
    <div class="card shadow-lg p-3">
        ...
        <form action="{{ route('facility.update', $facility) }}" method="post" class="form">
            @csrf
            @method('put')
                ...
                <div class="col-md-12">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select select2-default @error('property_id') is-invalid @enderror" id="property_id" name="property_id" required>
                            ...
                            @foreach($properties as $property)
                                <option value="{{ $property->id }}" @selected(old('property_id', $facility->property_id) == $property->id)>{{ $property->name }}</option>
                            @endforeach
                        </select>
                        @error('property_id')
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('name') is-invalid @enderror" type="text" id="name"
                            name="name" required value="{{ old('name', $facility->name) }}">
                        @error('name')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('icon') is-invalid @enderror" type="text" id="icon"
                            name="icon" value="{{ old('icon', $facility->icon) }}" placeholder="Misal: bx bx-wifi">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description"
                            name="description" rows="3">{{ old('description', $facility->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
